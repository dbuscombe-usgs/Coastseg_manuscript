...
### Performance
 CoastSeg enhances the image retrieval process from ```CoastSat``` by offering a more reliable and efficient download mechanism. It supports downloading multiple regions of interest in a single session and ensures downloads persist even over an unstable internet connectionShould a download error occur,CoastSeg briefly pauses before reconnecting to Google Earth Engine, ensuring the process doesn't halt completely.
 In cases where an image download fails repeatedly, the filename is logged to a report file located within the downloaded data's region folder. This report file tracks the status of all requested images from Google Earth Engine. CoastSeg's reliable image retrieval process enhances coastal monitoring by facilitating easier data management and collaboration, even in challenging internet conditions. 
