...
* Runup correction - add/subtract a height from tide, then apply horizontal correction
* Automated satellite image subpixel co-registration using the AROSICS package [@scheffler2017arosics]
* New toolbox for additional sources of imagery such as Planetscope [@doherty2022python]
* new models for Zoo workflow based on NDWI and MNDWI (add REFS)
* Docker

Other improvements that could be made include more (or more refined) outlier detection methods, image filtering procedures, etc. Such additions would all be easily possible without major changes to the existing ``CoastSeg`` API
